...
Map<String, dynamic> calculateTransaction(List<Expense> expenses) {
  final pastWeeksTransaction = expenses.where((expense) {
    final weeksAgo = DateTime.now().subtract(const Duration(days: 7 * 2));
    return expense.date.isAfter(weeksAgo);
  }).toList();
  ...
  for (var expense in pastWeeksTransaction) {
    final dayOfWeek = DateFormat('EEEE').format(expense.date);
    dailyTransaction.update(
        dayOfWeek,
            (existingValue) => expense.type == 'Expense' ? existingValue - expense.price : existingValue + expense.price,
        ifAbsent: () => expense.price);
  }

  return {
    'pastWeeksTransaction': pastWeeksTransaction,
    'dailyTransaction': dailyTransaction,
  };
}

LineChartData buildTLineChartData(Map<String, double> dailyExpenses, BuildContext context, List<Expense> expenses) {
